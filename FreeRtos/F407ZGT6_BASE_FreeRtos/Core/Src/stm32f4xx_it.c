...
void __attribute__((used)) HardFault_Handler_C(uint32_t *hardfault_args)
{
    ...
    SEGGER_RTT_Write(0, msg, len);  // 修复：使用msg而不是buffer，并使用len而不是strlen(msg)
...
void USART6_IRQHandler(void)
{
  ...
  {
    // 读取接收到的字节
    receivedByte = (uint8_t)(huart6.Instance->DR & 0x00FF);
    
    // 直接将字节发送到队列
    xQueueSendFromISR(Uart_Rx_Request_Queue, &receivedByte, &pxHigherPriorityTaskWoken);
    
    // 清除RXNE标志位（通常读取DR后自动清除）
  ...
  if(__HAL_UART_GET_FLAG(&huart6, UART_FLAG_IDLE ) != RESET) //中断中判断产生了空闲中断标志认为一轮结束
  {
    ...
    xSemaphoreGiveFromISR( BinaryRxSem_Handle, &pxHigherPriorityTaskWoken );//给出二值信号量
